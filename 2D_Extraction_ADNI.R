...
foreach(nifti_file_name = ad) %dopar%
  {
    ...
    print(image_id)
    folder <- paste("/home/dthomas/AD/ADNI/2D/AD_3T/", sub, "_", ses, image_id, sep="")
    dir.create(folder)
...
foreach(nifti_file_name = cn) %dopar%
  {
    ...
    ses <- str_extract(nifti_file_name, '(S_\\d*)')
    image_id <- str_extract(nifti_file_name, '(_I\\d*)')
    folder <- paste("/home/dthomas/AD/ADNI/2D/CN_3T/", sub, "_", ses, image_id, sep="")
    dir.create(folder)
...
ad <- list.files("ADNI_1_5_T_AD")
cn <- list.files("ADNI_1_5_T_CN")
...
setwd("/home/dthomas/AD/ADNI/ADNI_1_5_T_AD/")
start_time_ad <- Sys.time()
foreach(nifti_file_name = ad) %dopar%
  {
    t1 <- readnii(paste(nifti_file_name))
    ...
    image_id <- str_extract(nifti_file_name, '(_I\\d*)')
    print(image_id)
    folder <- paste("/home/dthomas/AD/ADNI/2D/AD_1_5T/", sub, "_", ses, image_id, sep="")
    dir.create(folder)
...
gc()
start_time_cn <- Sys.time()
setwd("/home/dthomas/AD/ADNI/ADNI_1_5_T_CN/")
foreach(nifti_file_name = cn) %dopar%
  {
    t1 <- readnii(paste(nifti_file_name))
    sub <- str_extract(nifti_file_name, '(ADNI_\\d*)')
    ses <- str_extract(nifti_file_name, '(S_\\d*)')
    image_id <- str_extract(nifti_file_name, '(_I\\d*)')
    folder <- paste("/home/dthomas/AD/ADNI/2D/CN_1_5T/", sub, "_", ses, image_id, sep="")
    dir.create(folder)
